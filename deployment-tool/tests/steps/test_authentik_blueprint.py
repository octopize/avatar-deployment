...
class TestAuthentikBlueprintStep:
    ...
    def test_collect_config(self, step):
        ...
        assert "AVATAR_AUTHENTIK_BLUEPRINT_DOMAIN" in config
        assert "AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID" in config
        assert "AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET" in config
        assert "AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI" in config
        assert "AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE" in config
    ...
    def test_collect_config_values(self, step):
        ...
        assert config["AVATAR_AUTHENTIK_BLUEPRINT_DOMAIN"] == "avatar.example.com"

        # Client ID and secret should be generated (64 hex chars)
        assert len(config["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID"]) == 64
        assert len(config["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET"]) == 64

        # Redirect URI should be built from domain
        assert (
            config["AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI"]
            == "https://avatar.example.com/api/login/sso/auth"
        )

        # License type should default to demo
        assert config["AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE"] == "demo"
    ...
    def test_collect_config_custom_values(self, tmp_path):
        ...
        config = {
            ...
            "AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID": "custom-client-id",
            "AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET": "custom-secret",
            "AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI": "https://custom.example.com/auth",
            "AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE": "trial",
        }
        step = AuthentikBlueprintStep(tmp_path, {}, config, interactive=False)
        ...
        assert result["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID"] == "custom-client-id"
        assert result["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET"] == "custom-secret"
        assert (
            result["AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI"]
            == "https://custom.example.com/auth"
        )
        assert result["AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE"] == "trial"
    ...
    def test_collect_config_unique_ids(self, tmp_path):
        ...
        assert (
            result1["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID"]
            != result2["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_ID"]
        )
        assert (
            result1["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET"]
            != result2["AVATAR_AUTHENTIK_BLUEPRINT_CLIENT_SECRET"]
        )
    ...
    def test_collect_config_updates_self_config(self, step):
        ...
        assert (
            step.config["AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI"]
            == config["AVATAR_AUTHENTIK_BLUEPRINT_API_REDIRECT_URI"]
        )
        assert (
            step.config["AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE"]
            == config["AVATAR_AUTHENTIK_BLUEPRINT_SELF_SERVICE_LICENSE"]
        )
