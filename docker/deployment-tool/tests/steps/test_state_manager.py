...
class TestDeploymentState:
    ...
    def state(self, temp_state_file):
        ...
        return DeploymentState(temp_state_file)
    ...
    def test_init_uses_default_steps(self, state):
        """Test that default steps are used when none provided."""
        assert len(state.steps) == len(DeploymentState.DEFAULT_STEPS)
        for step in DeploymentState.DEFAULT_STEPS:
            assert step in state.state["steps"]
    ...
    def test_save_creates_parent_directory(self, tmp_path):
        ...
        state = DeploymentState(nested_path)
        state.save()
    ...
    def test_load_existing_state(self, temp_state_file):
        ...
        state1 = DeploymentState(temp_state_file)
        state1.state["config"]["loaded"] = True
        state1.save()

        # Load existing state
        state2 = DeploymentState(temp_state_file)
