...
from octopize_avatar_deploy.input_gatherer import RICH_AVAILABLE, RichInputGatherer
...
class TestRichInputGatherer:
    ...
    def test_prompt_with_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            Prompt, "ask", lambda *args, **kwargs: kwargs.get("default", "")
        )
        gatherer = RichInputGatherer()
    ...
    def test_prompt_yes_no_default_true(self, monkeypatch):
        ...
        monkeypatch.setattr(
            Confirm, "ask", lambda *args, **kwargs: kwargs.get("default", True)
        )
        gatherer = RichInputGatherer()
    ...
    def test_prompt_yes_no_default_false(self, monkeypatch):
        ...
        monkeypatch.setattr(
            Confirm, "ask", lambda *args, **kwargs: kwargs.get("default", False)
        )
        gatherer = RichInputGatherer()
    ...
    def test_prompt_choice_with_default(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            IntPrompt, "ask", lambda *args, **kwargs: kwargs.get("default", 1)
        )
        gatherer = RichInputGatherer()
        choices = ["option1", "option2", "option3"]
...
def test_rich_printer_requires_rich():
    """Test RichPrinter raises ImportError if rich not available."""
    if not RICH_AVAILABLE:
        with pytest.raises(ImportError, match="Rich library not available"):
            RichPrinter()


def test_rich_input_gatherer_requires_rich():
    """Test RichInputGatherer raises ImportError if rich not available."""
    if not RICH_AVAILABLE:
        with pytest.raises(ImportError, match="Rich library not available"):
            RichInputGatherer()
