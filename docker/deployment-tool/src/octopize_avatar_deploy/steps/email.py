...
class EmailStep(DeploymentStep):
    ...
    def collect_config(self) -> dict[str, Any]:
        ...
        if provider == "smtp":
            ...
            config["SMTP_USE_TLS"] = self.config.get(
                "SMTP_USE_TLS", smtp_defaults["use_tls"]
            )
            config["SMTP_START_TLS"] = self.config.get(
                "SMTP_START_TLS", smtp_defaults["start_tls"]
            )
            config["SMTP_VERIFY"] = self.config.get(
                "SMTP_VERIFY", smtp_defaults["verify"]
            )
    ...
    def generate_secrets(self) -> dict[str, str]:
        ...
        if self.config.get("MAIL_PROVIDER") == "smtp":
            if self.interactive:
